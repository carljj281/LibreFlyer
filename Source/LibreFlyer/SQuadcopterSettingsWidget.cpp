...
void SQuadcopterSettingsWidget::Construct(const FArguments& InArgs) {
	...
	NewVectorEntry(
		SettingsBox.ToSharedRef(),
		FText::FromString("RC Rate"),
		//FVector::ZeroVector,
		FVector(
			currentControlRateProfile.rcRates[0],
			currentControlRateProfile.rcRates[1],
			currentControlRateProfile.rcRates[2]
		),
		FOnVectorValueChanged::CreateSP(this, &SQuadcopterSettingsWidget::RcRatesChanged)
	);
}
...
void SQuadcopterSettingsWidget::NewVectorEntry(TSharedRef<SVerticalBox> SettingsBox, const FText& EntryName, FVector InitialValue, FOnVectorValueChanged EntryDelegate)
{
	SettingsBox->AddSlot()
		.Padding(FMargin(0.f, 0.f, 0.f, 5.f))
		[
...
void SQuadcopterSettingsWidget::RcRatesChanged(FVector NewValue) {
	currentControlRateProfile.rcRates[0] = NewValue.X;
	currentControlRateProfile.rcRates[1] = NewValue.Y;
	currentControlRateProfile.rcRates[3] = NewValue.Z;
}

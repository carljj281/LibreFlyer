...
static	FcontrolRateConfig_s currentControlRateProfile = FcontrolRateConfig_s();
...
static float applyKissRates(const int axis, float rcCommandf, const float rcCommandfAbs) {
	...
	float kissRcCommandf = (powf(rcCommandf, 3.f) * rcCurvef + rcCommandf * (1 - rcCurvef)) * (currentControlRateProfile.rcRates[axis] / 1000.0f);
...
static float applyActualRates(const int axis, float rcCommandf, const float rcCommandfAbs) {
	...
	expof = rcCommandfAbs * (powf(rcCommandf, 5.f) * expof + rcCommandf * (1 - expof));

	const float centerSensitivity = currentControlRateProfile.rcRates[axis] * 10.0f;
	const float stickMovement = fmaxf(0.f, currentControlRateProfile.rates[axis] * 10.0f - centerSensitivity);
...
static float applyQuickRates(const int axis, float rcCommandf, const float rcCommandfAbs) {
	...
	const uint16_t maxDPS = fmaxf(currentControlRateProfile.rates[axis] * 10, rcRate);
	const float expof = currentControlRateProfile.rcExpo[axis] / 100.0f;
	...
	if (currentControlRateProfile.quickRatesRcExpo) {
		curve = powf(rcCommandf, 3.f) * expof + rcCommandf * (1 - expof);
...
static float applyEasyRates(const int axis, float rcCommandf, const float rcCommandfAbs) {
	// rates = CenterStickPrecision
	// rcExpo = OuterStickTransition
	// rcRates = MaxRotationSpeed
	rcCommandf *= currentControlRateProfile.rcRates[axis] * 
		(
			currentControlRateProfile.rates[axis] * 
			powf(rcCommandfAbs, 50.f * powf(currentControlRateProfile.rcExpo[axis], 7.f)) + 
			(1.f - currentControlRateProfile.rates[axis])
		);
	return rcCommandf;
}
struct FEasyRates {
...
static FEasyRates EasyRates = FEasyRates();
